...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.HarperCollins
{
    class HarperCollins_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_HarperCollins_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_HarperCollins(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_HarperCollins_Ebook"].ToString());
        ...
        public DataTable ISBN(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_ISBN, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_product_productidentifier_List.Count; a++)
            {
                ...
                {
                    i++;
                    if (product.obj_product_productidentifier_List[a].b244_product_productidentifier == "9780061743153")
                    {
                        string asdfsdf = "asdfasdsaas";
                    }
                    dr["MetaDataID"] = MetaDataID;
                    dr["ProductID"] = productCount;
                    dr["RowCnt"] = i;
                ...
                for (int b = 0; b < product.obj_product_supplydetail_List[a].obj_supplydetail_price_List.Count; b++)
                {
                    ...
                    if (product.obj_product_supplydetail_List[a].obj_supplydetail_price_List[b].j152_product_supplydetail_price.ToLower() == "cad")
                    {
                        ...
                        //    {
                                if (!string.IsNullOrEmpty(product.obj_product_supplydetail_List[a].obj_supplydetail_price_List[b].j148_product_supplydetail_price))
                                {
                                    if (product.obj_product_supplydetail_List[a].obj_supplydetail_price_List[b].j148_product_supplydetail_price.ToLower() == "41")
                                    {
