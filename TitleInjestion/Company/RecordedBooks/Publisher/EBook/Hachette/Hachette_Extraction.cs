...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.Hachette
{
    class Hachette_Extraction
    {
      ...
      System.Windows.Forms.Label lbl_Message)
        ...
        public bool Process_Hachette(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_Hachette_Ebook"].ToString());
        ...
        public DataTable UnAbridged(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_UnAbridged, int MetaDataID, int productCount)
        {
            ...
            {
                if (!string.IsNullOrEmpty(product.obj_b056_List[a].ToString()))
                {
                    if (product.obj_b056_List[a].ToLower().ToString() != "ill")
                    {
        ...
        public DataTable DigitalFormat_b333(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_DigitalFormat_b333, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_b333_List.Count; a++)
            {
                ...
                if (!string.IsNullOrEmpty(product.obj_b333_List[a].ToString()))
                {
