...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.HMH
{
    class HMH_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_HMH_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
            ...
            System.Windows.Forms.Label lbl_Message)
        {
            ...
            try
            {
                ...
                DataTable dt_EbookVersion_b014 = new DataTable("EbookVersion_b014");
                #region 'Columns Declaration'

                dt_EbookVersion_b014.Columns.Add("MetaDataID", typeof(int));
                dt_EbookVersion_b014.Columns.Add("ProductID", typeof(int));
                dt_EbookVersion_b014.Columns.Add("RowCnt", typeof(int));
                dt_EbookVersion_b014.Columns.Add("EbookVersion_b014", typeof(string));
                ...
                if (MetaDataID > 0)
                {
                    for (int i = 0; i < fileinfo_1.obj_product_List.Count; i++)
                    {
                        ...
                        #region 'b014'
                        Step = "b014";
                        dt_EbookVersion_b014 = EbookVersion_b014(fileinfo_1.obj_product_List[i], dt_EbookVersion_b014, MetaDataID, (i + 1));
                        #endregion
                    ...
                    if (result)
                    {
                        result = InsertRecords(dt_EbookVersion_b014, "RB");
                        Insertion_Label(lbl_Insert, count);
                    }
                    count--;
        ...
        public bool Process_HMH(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_HMH_Ebook"].ToString());
        ...
        public DataTable EbookVersion_b014(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_EbookVersion_b014, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_b014_List.Count; a++)
            {
                if (!string.IsNullOrEmpty(product.obj_b014_List[a]))
                {
                    if (product.obj_b014_List[a].ToLower() == "epub")
                    {
                        DataRow dr = dt_EbookVersion_b014.NewRow();

                        dr["MetaDataID"] = MetaDataID;
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = (a + 1);
                        dr["EbookVersion_b014"] = "Reflowable";

                        dt_EbookVersion_b014.Rows.Add(dr);
                       
                    }
                    else if (product.obj_b014_List[a].ToLower() == "pdf")
                    {
                        DataRow dr = dt_EbookVersion_b014.NewRow();
                        ...
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = (a + 1);
                        dr["EbookVersion_b014"] = "PDF";

                        dt_EbookVersion_b014.Rows.Add(dr);
                    }
                    else if (product.obj_b014_List[a].ToLower() == "open ebook format")
                    {
                        DataRow dr = dt_EbookVersion_b014.NewRow();
                    ...
                    else if (product.obj_b014_List[a].ToLower() == "standard fixed layout ebook")
                    {
                        DataRow dr = dt_EbookVersion_b014.NewRow();
                        ...
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = (a + 1);
                        dr["EbookVersion_b014"] = "Fixed Layout";

                        dt_EbookVersion_b014.Rows.Add(dr);
                    }
                    else if (product.obj_b014_List[a].ToLower() == "standard fixed layout epub")
                    {
                        DataRow dr = dt_EbookVersion_b014.NewRow();
                    ...
                    else
                    {
                         ...
                         dr["ProductID"] = productCount;
                         dr["RowCnt"] = (a + 1);
                         dr["EbookVersion_b014"] = product.obj_b014_List.ToString();
