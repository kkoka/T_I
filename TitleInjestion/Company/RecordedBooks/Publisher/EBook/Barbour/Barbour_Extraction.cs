...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.Barbour
{
    class Barbour_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_Barbour_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_Barbour(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_BarbourEbook"].ToString());
            ...
            for (int a = 0; a < product.obj_b014_List.Count; a++)
            {
                if (!string.IsNullOrEmpty(product.obj_b014_List[a]))
                {
                    if (product.obj_b014_List[a].ToLower() == "epub")
                    {
                        ...
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = (a + 1);
                        dr["EbookVersion_b014"] = product.obj_b014_List[a];
