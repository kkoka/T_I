...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.HarperCollinsUK
{
    class HarperCollinsUK_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_HarperCollinsUK_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_HarperCollinsUK(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_HarperCollinsUK_Ebook"].ToString());
        ...
        public DataTable Imprint(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_Imprint, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_imprint_List.Count; a++)
            {
                ...
                if (string.IsNullOrEmpty(product.obj_imprint_List[a].b241_product_imprint)
                    &&
                    string.IsNullOrEmpty(product.obj_imprint_List[a].b242_product_imprint)
                    &&
                    string.IsNullOrEmpty(product.obj_imprint_List[a].b243_product_imprint)
                    )
