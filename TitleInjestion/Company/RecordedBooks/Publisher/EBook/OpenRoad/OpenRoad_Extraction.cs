...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EBook.OpenRoad
{
    class OpenRoad_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_OpenRoad_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_OpenRoad(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_OpenRoad_Ebook"].ToString());
        ...
        public DataTable b213(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_b213, int MetaDataID, int productCount)
        {
            ...
            {
                if (!string.IsNullOrEmpty(product.obj_b213_List[a]))
                {
                    if (product.obj_b213_List[a].ToLower() == "pdf")
                    {
                    ...
                    else if(product.obj_b213_List[a].ToLower() == "open ebook format")
                    {
                        dr["MetaDataID"] = MetaDataID;
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = i;
