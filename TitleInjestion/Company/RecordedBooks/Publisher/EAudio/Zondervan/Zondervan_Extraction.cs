...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EAudio.Zondervan
{
    class Zondervan_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_Zondervan_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
            ...
            try
            {
                ...
                if (MetaDataID > 0)
                {
                    for (int i = 0; i < fileinfo_1.obj_product_List.Count; i++)
                    {
                        ...
                        #region 'Unabridged_b014'
                        Step = "Unabridged_b014";
        ...
        public bool Process_Zondervan_EAudio(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_Zondervan_EAudio"].ToString());
        ...
        public DataTable Publisher(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_Publisher, int MetaDataID, int productCount)
        {
            ...
            //{
                ...
                dr["ProductID"] = productCount;
                dr["RowCnt"] = 1;
                dr["Publisher_b081"] = "Zondervan"; //  product.obj_product_publisher_List[a].b081_product_publisher;
        ...
        public DataTable EbookVersion_b014(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_EbookVersion_b014, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_b014_List.Count; a++)
            {
                ...
                if (!string.IsNullOrEmpty(product.obj_b014_List[a]))
                {
                    ...
                    if (product.obj_b014_List[a].ToLower() == "download - audio, unabridged")
                    {
                        b014 = "Unabridged";
                    }
                    else if (product.obj_b014_List[a].ToLower() == "download - audio, abridged")
                    {
                        b014 = "Abridged";
                    }
                    else if (product.obj_b014_List[a].ToLower() == "download - audio")
                    {
                        b014 = "Unabridged";
                    }
                    else
                    {
                        b014 = product.obj_b014_List[a].ToString();
                    }
