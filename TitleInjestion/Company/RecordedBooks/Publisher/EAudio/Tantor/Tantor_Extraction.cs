...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EAudio.Tantor
{
    class Tantor_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_Tantor_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_Tantor_EAudio(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_Tantor_EAudio"].ToString());
        ...
        public DataTable Publisher(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_Publisher, int MetaDataID, int productCount)
        {
            ...
            //{
                ...
                dr["ProductID"] = productCount;
                dr["RowCnt"] = 1;
                dr["Publisher_b081"] = "Tantor Media, Inc"; // product.obj_product_publisher_List[a].b081_product_publisher;
        ...
        public DataTable DRMFlag_x317(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_DRMFlag_x317, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_x317_List.Count; a++)
            {
                if (!string.IsNullOrEmpty(product.obj_x317_List[a]))
                {

                    if (product.obj_x317_List[a].Length > 0)
                    {
                        ...
                        dr["ProductID"] = productCount;
                        dr["RowCnt"] = (a + 1);
                        dr["DRMFlag_x317"] = product.obj_x317_List[a].ToString();
