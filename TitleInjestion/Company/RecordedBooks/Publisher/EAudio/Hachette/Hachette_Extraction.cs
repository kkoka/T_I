...
namespace TitleInjestion.Company.RecordedBooks.Publisher.EAudio.Hachette
{
    class Hachette_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool RB_Hachette_Extraction(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_Hachette_EAudio(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("RB", ConfigurationSettings.AppSettings["RB_Process_Hachette_EAudio"].ToString());
        ...
        public DataTable ISBN(TitleInjestion.Company.RecordedBooks.Onix_2_Short_Definition.product product, DataTable dt_ISBN, int MetaDataID, int productCount)
        {
            ...
            {
                if (!string.IsNullOrEmpty(product.obj_product_productidentifier_List[a].b233_product_productidentifier))
                {
                    if (product.obj_product_productidentifier_List[a].b233_product_productidentifier.ToLower() == "library isbn")
                    {
                        i++;
