...
namespace TitleInjestion.Company.WFHowes.Publisher.EAudio.Tantor_WFH
{
    class TantorWFH_Extraction
    {

        public List<string> d101_productID = new List<string>();

        public bool Tantor_WFH_Extraction(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_TantorWFH(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("WFH", ConfigurationSettings.AppSettings["WFH_Process_TantorWFH_eAudio_UK"].ToString());
                    ...
                    if (Convert.ToInt32(product.obj_b073_List[a].ToString()) == 1)
                    {
                        if (!string.IsNullOrEmpty(product.obj_b073_List[a].ToString()))
                        {
                            ...
                            if (Convert.ToInt32(product.obj_b073_List[a].ToString()) == 1)
                            {
                                ...
                                //Convert.ToInt32(product.obj_b073_List[a].ToString()) == 2 ||
        ...
        public DataTable MinAge(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.product product, DataTable dt_RH_MinAge, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_productaudiencerange_List.Count; a++)
            {
                ...
                {
                    if (product.obj_productaudiencerange_List[a].b074_product.ToString() == "11")
                    {
