...
namespace TitleInjestion.Company.WFHowes.Publisher.EAudio.Michael_OMara
{
    class Michael_OMara_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool MichaelOMara_Extraction(TitleInjestion.Company.WFHowes.Onix_2_Reference_Definiton.ONIXMessage fileinfo_1, int pubid, string FileName, string MediaType,
        ...
        public bool Process_MichaelOMara(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("WFH", ConfigurationSettings.AppSettings["WFH_Process_MichaelOMara_eAudio_UK"].ToString());
        ...
        public DataTable TotalRunTime(TitleInjestion.Company.WFHowes.Onix_2_Reference_Definiton.Product product, DataTable dt_TotalRunTime, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_product_Extent_List.Count; a++)
            {

                if (!string.IsNullOrEmpty(product.obj_product_Extent_List[a].ExtentType_Product_Extent))
                {
                    if (product.obj_product_Extent_List[a].ExtentType_Product_Extent.Trim() == "09")
                    {
                        if (!string.IsNullOrEmpty(product.obj_product_Extent_List[a].ExtentUnit_Product_Extent))
                        {
                            if (product.obj_product_Extent_List[a].ExtentUnit_Product_Extent.Trim() == "05")
                            {

                                if (!string.IsNullOrEmpty(product.obj_product_Extent_List[a].ExtentValue_Product_Extent))
                                {
                                    ...
                                    dr["ProductID"] = productCount;
                                    dr["RowCnt"] = (a + 1);
                                    dr["TotalRunTimeCode_b218"] = product.obj_product_Extent_List[a].ExtentType_Product_Extent.ToString();
                                    dr["TotalRunTime_b219"] = product.obj_product_Extent_List[a].ExtentValue_Product_Extent.ToString();
                                    dr["TotalRunTimeUnit_b220"] = product.obj_product_Extent_List[a].ExtentUnit_Product_Extent.ToString();
        ...
        {
            //<TitleOfSeries> if not found then use <TitleText>
            ...
            for (int a = 0; a < product.obj_ProductSeries_List.Count; a++)
            {
                ...
                SeriesNumber = StringReplace(product.obj_ProductSeries_List[a].NumberWithinSeries_ProductSeries, "No.", "");
                ...
                {
                  SeriesName = product.obj_ProductSeries_List[a].TitleOfSeries_ProductSeries;              
                }
                else
                {
                    for(int b= 0; b < product.obj_ProductSeries_List[a].obj_productSeries_Title_List.Count; b++)
                    {

                        if (product.obj_ProductSeries_List[a].obj_productSeries_Title_List[b].TitleType_Product_Series_Title == "1")
                        {
                            if(SeriesName.Length== 0)
                            { 
                                SeriesName = product.obj_ProductSeries_List[a].obj_productSeries_Title_List[b].TitleText_Product_Series_Title;
                            }
                        }                   
                    }
                }
                ...
                dr["SeriesNumber_b019"] = SeriesNumber;
        ...
        public DataTable ProductFormCode_b012(TitleInjestion.Company.WFHowes.Onix_2_Reference_Definiton.Product product, DataTable dt_RH_ProductFormCode_b012, int MetaDataID, int productCount)
        {
            ...
            {
                if(product.obj_ProductForm_List[a].ToLower() == "aj")
                {
