...
namespace TitleInjestion.Company.WFHowes.Publisher.Ebook.Bloomsbury_UK
{
    class Bloomsbury_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool Bloomsbury_UK_Extraction(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.ONIXmessage fileinfo_1, int pubid, string FileName, string MediaType,
            ...
            System.Windows.Forms.Label lbl_Message)
        {
            ...
            try
            {
                ...
                //DataTable dt_Audience_b073 = new DataTable("Audience_b073");
                //#region 'Columns Declaration'

                //dt_Audience_b073.Columns.Add("MetaDataID", typeof(int));
                //dt_Audience_b073.Columns.Add("ProductID", typeof(int));
                //dt_Audience_b073.Columns.Add("RowCnt", typeof(int));
                //dt_Audience_b073.Columns.Add("Audience_b073", typeof(string));
                ...
                if (MetaDataID > 0)
                {
                    for (int i = 0; i < fileinfo_1.obj_product_List.Count; i++)
                    {
                        ...
                        Step = "dt_RH_Audience";
                        dt_Audience = Audience_b073(fileinfo_1.obj_product_List[i], dt_Audience, MetaDataID, (i + 1));
                        Step = "dt_RH_Audience";
        ...
        public bool Process_Bloomsbury_UK(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("WFH", ConfigurationSettings.AppSettings["WFH_Process_Bloomsbury_Ebook_UK"].ToString());
        ...
        public DataTable Description(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.product product, DataTable dt_Description, int MetaDataID, int productCount)
        {
            ...
            for (int a = 0; a < product.obj_product_othertext_List.Count; a++)
            {
                if (product.obj_product_othertext_List[a].d102_product_othertext == "03" ||
                    ...
                    product.obj_product_othertext_List[a].d102_product_othertext == "02" //|| product.obj_product_othertext_List[a].d102_product_othertext == "25"
                    )
                {
        ...
        public DataTable Audience_b073(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.product product, DataTable dt_Audience_b073, int MetaDataID, int productCount)
        {
            //dr["FileName"] = fileinfo_1.obj_FileInfo_List[0].FileName_FileInfo;
            //dr["pubId"] = fileinfo_1.obj_FileInfo_List[0].PubID_FileInfo;
            //dr["FileType"] = fileinfo_1.obj_FileInfo_List[0].FileType_FileInfo;
            ...
            {
                //if (!string.IsNullOrEmpty(product.obj_b073_List[a].ToString()))
                //{
                    //if (Convert.ToInt32(product.obj_b073_List[a].ToString()) == 1)
                    //{
                        //if (!string.IsNullOrEmpty(product.obj_b073_List[a].ToString()))
                        //{
                            ...
                            else if (Convert.ToInt32(product.obj_b073_List[a].ToString()) == 2 ||
                                     Convert.ToInt32(product.obj_b073_List[a].ToString()) == 3 ||
                                     Convert.ToInt32(product.obj_b073_List[a].ToString()) == 4)
                            {
                            ...
                            {
                                DataRow dr = dt_Audience_b073.NewRow();
        ...
        public DataTable ProductFormCode_b012(TitleInjestion.Company.WFHowes.Onix_2_Short_Definition.product product, DataTable dt_ProductFormCode_b012, int MetaDataID, int productCount)
        {
            ...
            {
                if (!string.IsNullOrEmpty(product.obj_b012_List[a].ToString()))
                {
