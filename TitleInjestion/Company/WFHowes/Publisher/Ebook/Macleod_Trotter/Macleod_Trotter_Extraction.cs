...
namespace TitleInjestion.Company.WFHowes.Publisher.Ebook.Macleod_Trotter
{
    class Macleod_Trotter_Extraction
    {
        public List<string> d101_productID = new List<string>();

        public bool Macleod_Trotter_UK_Extraction(int pubid, string FileName_Path, string FileName, string MediaType,
            ...
            System.Windows.Forms.Label lbl_Message)
        {
            ...
            try
            {
                ...
                foreach (DataColumn column in dt_TitleCollection.Columns)
                {
                    ...
                    else if (column.ColumnName == "PageCount")
                    {
                        Titles_PageCount = true;
                    }
                ...
                else
                {
                    ...
                    if (!Titles_PageCount)
                    {
                        false_values = false_values + ", PAGECOUNT";
                    }
                    if (!Titles_Territory)
                    {
        ...
        public bool Process_Macleod_Trotter_UK(string Company, System.Windows.Forms.Label lbl_Processing)
        {
            bool result = false;
            ...
            SQLFunction sqlfunction = new SQLFunction();
            result = sqlfunction.ExecuteProc("WFH", ConfigurationSettings.AppSettings["WFH_Process_Macleod_Trotter_Ebook_UK"].ToString());
